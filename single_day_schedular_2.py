"""
GPT-4 PROMPT:
Now add track names to each talk.  We have 5 tracks : "Java", "AI", "Security", "Culture", "Server Side Java".
The talks must be evenly spread over the talks.
Show also the track name in the print out.
"""
...
for track, count in tracks.items():
    for room in rooms:
        for idx, time_slot in enumerate(time_slots):
            var = model.NewIntVar(0, count, f'{track}_{room}_{time_slot}')
            track_vars[(track, room, time_slot)] = var
            model.Add(track_vars[(track, room, time_slot)] == sum(talk_vars[(talk, room, time_slot)] for talk in talks if talk[2] == track))

for idx, time_slot in enumerate(time_slots):
    model.Add(sum(track_vars[(track, room, time_slot)] for room in rooms for track in tracks) <= 4)
