"""
GPT-4 PROMPT:
A speaker can have multiple talks, if so make sure the speaker doesn't give two talks at the same time.
Skip the dummy setup and only show the schedule pyhon logic
"""
...
talks = [
    ...
    ('Talk 28', 'Speaker 1', 'Security'),
    ('Talk 29', 'Speaker 1', 'Culture')
]
...
for speaker in set(talk[1] for talk in talks):
    ...
    for time_slot in time_slots:
        model.Add(sum(talk_vars[(talk, room, time_slot)] for talk in speaker_talks for room in rooms) <= 1)
